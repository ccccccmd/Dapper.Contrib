...
namespace Dapper.Contrib.Extensions
{
    public static partial class SqlMapperExtensions
    {
            ...
            IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            if (!type.IsInterface)
                return (await connection.QueryAsync<T>(sql, dynParams, transaction, commandTimeout)
                    .ConfigureAwait(false)).FirstOrDefault();

            if (!((await connection.QueryAsync<dynamic>(sql, dynParams).ConfigureAwait(false)).FirstOrDefault() is
        ...
        /// <typeparam name="T">Type to be updated</typeparam>
        /// <param name="connection">Open SqlConnection</param>
        /// <param name="parameters">parameters,must have at least one [Key] or [ExplicitKey] property</param>
        /// <param name="transaction">The transaction to run under, null (the default) if none</param>
        /// <param name="commandTimeout">Number of seconds before command execution timeout</param>
        /// <returns>true if updated, false if not found or not modified (tracked entities)</returns>
        public static async Task<bool> DynamicParametersUpdateAsync<T>(this IDbConnection connection, object parameters,
            IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            var type = typeof(T);
